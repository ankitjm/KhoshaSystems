...
const dataPoints = [
  { value: "<0.5%", label: "Shrinkage" },
  { value: "85–95%", label: "Scheme Recovery" },
  { value: "1–2 hrs", label: "GST Time / Week" },
  { value: "2–3 min", label: "Billing Speed" },
];
...
const tileClass = "rounded-2xl border border-stone-200 bg-white hover:border-bronze-300 glow-bronze transition-all duration-300";
const creamTileClass = "rounded-2xl border border-[#E0CCAD]/60 bg-[#FAF6F1] transition-all duration-300";
  ...
  return (
    <div>
      ...
      <Section className="bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="mb-10 sm:mb-12">
            <span className="text-bronze-600 font-semibold tracking-widest uppercase text-sm block mb-2">What Is RetailerOS</span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif text-stone-900">The Overview <span className="bronze-gradient-text">Dashboard.</span></h2>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              ...
              <div>
                <h3 className="text-stone-900 font-semibold text-lg sm:text-xl mb-3">Built for Indian retail counters</h3>
            ...
            {dataPoints.map((stat, i) => (
              <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                className={`p-5 sm:p-6 flex flex-col justify-center ${creamTileClass}`}>
                <div className="text-2xl sm:text-3xl font-bold text-stone-900 mb-1">{stat.value}</div>
                <div className="text-bronze-600 text-[10px] sm:text-xs uppercase tracking-wider">{stat.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <Section className="bg-stone-50">
        <div className="max-w-6xl mx-auto">
          <div className="mb-10 sm:mb-12">
            <span className="text-bronze-600 font-semibold tracking-widest uppercase text-sm block mb-2">Inside RetailerOS</span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif text-stone-900">The Modules You'll <span className="bronze-gradient-text">Use Every Day.</span></h2>
          </div>
      ...
      <Section className="bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10 sm:mb-12">
            <span className="text-bronze-600 font-semibold tracking-widest uppercase text-sm block mb-2">The Full Platform</span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif text-stone-900">23 Modules, <span className="bronze-gradient-text">Seven Categories.</span></h2>
            <p className="text-stone-500 max-w-2xl mx-auto mt-3">Verified against retaileros.in — the complete system beyond the daily counter view.</p>
          </div>
      ...
      <Section className="bg-stone-50">
        <div className="max-w-6xl mx-auto">
          <div className="mb-10 sm:mb-12">
            <span className="text-bronze-600 font-semibold tracking-widest uppercase text-sm block mb-2">Built For</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {personas.map((p, i) => (
              <motion.div key={p.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                className={`p-5 sm:p-6 ${tileClass}`}>
                <p.icon size={20} className="text-bronze-600 mb-3" />
                <h4 className="text-stone-900 font-medium text-sm mb-1">{p.label}</h4>
                <p className="text-stone-500 text-xs leading-relaxed">{p.desc}</p>
              </motion.div>
            ))}
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }}
              className={`col-span-1 sm:col-span-3 lg:col-span-1 p-5 sm:p-6 flex flex-col justify-center ${creamTileClass}`}>
              <Quote size={20} className="text-bronze-500 mb-3" />
              <p className="text-stone-700 text-sm italic leading-relaxed mb-3">"We bill over ₹2 crore a month across four counters, with every scheme applied automatically."</p>
              <div className="text-stone-900 text-xs font-medium">Mohammed Iqbal</div>
              <div className="text-bronze-600 text-[10px] uppercase tracking-widest">Electronics Retailer, 4 Counters</div>
            </motion.div>
          </div>
        </div>
      </Section>
      ...
      <Section className="bg-white">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10 sm:mb-12">
            <span className="text-bronze-600 font-semibold tracking-widest uppercase text-sm block mb-2">Subscription Packs</span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif text-stone-900">Pick Your Pack.</h2>
          </div>
          <div className="grid sm:grid-cols-3 gap-4">
            {plans.map((plan, i) => (
              <motion.div key={plan.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className={`p-6 flex flex-col ${plan.highlight ? 'border-2 border-bronze-400' : ''} ${tileClass}`}>
