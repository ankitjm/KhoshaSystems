...
const AnimatedStat: React.FC<{ value: string; label: string; delay: number }> = ({ value, label, delay }) => {
  ...
  return (
    ...
    >
      <div className="text-base sm:text-2xl md:text-3xl font-serif font-bold text-white">{value}</div>
      <div className="text-[8px] sm:text-[10px] text-white/50 uppercase tracking-wide sm:tracking-widest mt-0.5 whitespace-nowrap">{label}</div>
    </motion.div>
  );
};
...
export const DemoHero: React.FC<{ align?: 'left' | 'center' | 'right' }> = ({ align = 'center' }) => {
  ...
  const statsMarginClass = align === 'left' ? 'mr-auto' : align === 'right' ? 'ml-auto' : 'mx-auto';

  return (
    <header ref={containerRef} role="banner" aria-label="Khoshà Systems - Web Apps, SaaS Products & AI Transformation" className={`relative min-h-screen w-full flex flex-col ${itemsClass} justify-center overflow-hidden pb-20 sm:pb-24`}>
      ...
      <motion.div className={`relative z-10 ${textAlignClass} max-w-5xl px-5 sm:px-6 pt-20 sm:pt-24`}>
        <motion.div
          ...
          className={`flex ${justifyClass} mb-5 sm:mb-6`}
        >
        ...
        <motion.p
          ...
          className={`text-sm sm:text-base md:text-lg text-white/60 max-w-2xl ${statsMarginClass} mb-5 sm:mb-6 font-light leading-relaxed px-2`}
        >
        ...
        <motion.div
          ...
          className={`flex flex-wrap ${justifyClass} gap-2 mb-6 sm:mb-8 px-2`}
        >
        ...
        <motion.div
          ...
          className={`flex flex-col sm:flex-row gap-2.5 sm:gap-3 ${justifyClass} ${itemsClass} sm:items-center px-4 sm:px-0`}
        >
          <Link to="/contact"
            className="group px-6 sm:px-8 py-3 sm:py-3.5 bg-bronze-600 text-white font-medium uppercase tracking-widest text-xs sm:text-sm hover:bg-bronze-500 transition-colors duration-300 rounded text-center">
            <span className="flex items-center justify-center gap-2">
              Start a Project <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
            </span>
          </Link>
          <Link to="/products"
            className="px-6 sm:px-8 py-3 sm:py-3.5 border border-white/30 text-white font-medium uppercase tracking-widest text-xs sm:text-sm hover:border-bronze-400 hover:bg-white/10 transition-all duration-300 rounded text-center backdrop-blur-sm">
            Explore Products
          </Link>
        </motion.div>
      </motion.div>
